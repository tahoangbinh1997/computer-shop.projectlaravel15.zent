...
@section('content')
	...
	<div class="page-title">                    
		...
		<a class="btn btn-primary" data-toggle="modal" href='#user_add' style="float: right;">Thêm mới nhân viên</a>
	</div>
	<!-- END PAGE TITLE -->                
	...
	<script type="text/javascript">
		$(document).ready(function() {
			...
			var table = $('#users-table').DataTable({  //Datatable
				processing: true,
				serverSide: true,
				ajax: '{!! route('admin.admins.active.data') !!}',
				columns: [
					{ data: 'id', name: 'id' },
					{ data: 'name', name: 'name' },
			...
			Dropzone.options.formUserAddUpload = {  //Dropzone của bảng thêm mới nhân viên
				url: '{{route('admin.admins.active.upload.image')}}',
				maxFiles: 1,
				...
				init: function () {
					...
					this.on("complete",function(file){
						$('#add_user_pic').val("images/admin/"+file.name) // đường dẫn đến thư mục chứa ảnh của user
					})
					...
					this.on("removedfile", function (file) {
						...
						currentFile = null;
						$.ajax({
							url: '{{route('admin.admins.active.delete.image')}}',
							type: 'POST',
							data: {
								add_user_pic: file.name,
								add_user_pic_url: "images/admin/"+file.name,
			...
			$('.btn_add').click(function(event){ //click vào nút thêm mới ở modal thêm mới nhân viên
				event.preventDefault();
				$.ajax({
					url: '{{route('admin.admins.active.store')}}',
					type: 'POST',
				...
				.done(function(response) {
					...
					if (response.error) {
						...
						if (response.errors.add_permission) {
							$('select[name="add_permission"]').parent('div.input-group').addClass('has-error')
							$('select[name="add_permission"]').parent('div.input-group').removeClass('has-success')
							$('select[name="add_permission"]').parent('div.input-group').append('<span class="glyphicon glyphicon-remove form-control-feedback" style="left:91%;top:-2px;"></span>')
							$('select[name="add_permission"]').parent('div.input-group').find('span.glyphicon-ok').remove()
						}else {
							$('select[name="add_permission"]').parent('div.input-group').removeClass('has-error')
							$('select[name="add_permission"]').parent('div.input-group').addClass('has-success')
							$('select[name="add_permission"]').parent('div.input-group').find('span.glyphicon-remove').remove()
							$('select[name="add_permission"]').parent('div.input-group').append('<span class="glyphicon glyphicon-ok form-control-feedback" style="left:91%;top:-2px;"></span>')
						}
					}
			...
			$('#users-table tbody').on('click', '.btn_deactive', function(event) {
				event.preventDefault();

				var url = $(this).data('url');

				Swal({
					title: 'Bạn có chắc là muốn hủy kích hoạt?',
					text: "Bạn chắc là muốn hủy kích hoạt nhân viên này chứ!",
					type: 'warning',
					...
					if (result.value) {
						Swal(
							'Hủy kích hoạt nhân viên thành công!',
							'nhân viên sẽ được chuyển vào phần nhân viên đã bị hủy kích hoạt',
							'success'
							)
						$('button.swal2-confirm').on('click', function(event) {
							...
							.done(function(response) {
								if (response.deactive_success) {
									Command: toastr["warning"](""+response.deactive_success+""),
								...
								if (response.deactive_error) {
									Command: toastr["warning"](""+response.deactive_error+""),
			...
			$('#users-table tbody').on('click', '.btn_delete', function(event) {
				...
				Swal({
					...
					text: "Bạn chắc là muốn xóa nhân viên này chứ!",
					type: 'warning',
					showCancelButton: true,
					confirmButtonColor: '#3085d6',
					cancelButtonColor: '#d33',
					...
					if (result.value) {
						Swal(
							...
							'nhân viên sẽ được chuyển vào phần nhân viên đã bị xóa',
							'success'
							)
						$('button.swal2-confirm').on('click', function(event) {
							$.ajax({
