	...
	<script type="text/javascript">
		$(document).ready(function() {
			...
			var table = $('#users-table').DataTable({  //Datatable
				processing: true,
				serverSide: true,
				ajax: '{!! route('admin.admins.deactive.data') !!}',
				columns: [
					{ data: 'id', name: 'id' },
					{ data: 'name', name: 'name' },
			...
			Dropzone.options.formUserAddUpload = {  //Dropzone của bảng thêm mới nhân viên
				url: '{{route('admin.admins.deactive.upload.image')}}',
				maxFiles: 1,
				...
				init: function () {
					...
					this.on("removedfile", function (file) {
						...
						currentFile = null;
						$.ajax({
							url: '{{route('admin.admins.deactive.delete.image')}}',
							type: 'POST',
							data: {
			...
			$('.btn_add').click(function(event){ //click vào nút thêm mới ở modal thêm mới nhân viên
				...
				$.ajax({
					url: '{{route('admin.admins.deactive.store')}}',
					type: 'POST',
			...
			$('#users-table tbody').on('click', '.btn_active', function(event) {
				...
				Swal({
					...
					text: "Bạn chắc là muốn kích hoạt nhân viên này chứ!",
					type: 'warning',
						...
						$('button.swal2-confirm').on('click', function(event) {
							...
							.done(function(response) {
								if (response.active_success) {
									Command: toastr["warning"](""+response.active_success+""),
