	...
	<script type="text/javascript">
		$(document).ready(function() {
			...
			var table = $('#users-table').DataTable({  //Datatable
				processing: true,
				serverSide: true,
				ajax: '{!! route('admin.user.active.data') !!}',
				columns: [
					{ data: 'id', name: 'id' },
					{ data: 'name', name: 'name' },
			...
			Dropzone.options.formUserAddUpload = {  //Dropzone của bảng thêm mới người dùng
				url: '{{route('admin.user.active.upload.image')}}',
				maxFiles: 1,
				...
				init: function () {
					...
					this.on("removedfile", function (file) {
						...
						currentFile = null;
						$.ajax({
							url: '{{route('admin.user.active.delete.image')}}',
							type: 'POST',
							data: {
			...
			$('.btn_add').click(function(event){ //click vào nút thêm mới ở modal thêm mới người dùng
				...
				$.ajax({
					url: '{{route('admin.user.active.store')}}',
					type: 'POST',
			...
			$('#users-table tbody').on('click', '.btn_deactive', function(event) {
				...
				Swal({
					...
					text: "Bạn chắc là muốn hủy kích hoạt người dùng này chứ!",
					type: 'warning',
					...
					if (result.value) {
						Swal(
							...
							'Người Dùng sẽ được chuyển vào phần người dùng đã bị hủy kích hoạt',
							'success'
							)
