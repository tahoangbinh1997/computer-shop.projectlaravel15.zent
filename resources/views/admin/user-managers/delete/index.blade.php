...
@section('content')
	...
	<div class="page-title">                    
		<h2><span class="fa fa-arrow-circle-o-left"></span> Người Dùng Bị Xóa</h2>
	</div>
	<!-- END PAGE TITLE -->                
	...
	<script type="text/javascript">
		$(document).ready(function() {
			...
			var table = $('#users-table').DataTable({  //Datatable
				processing: true,
				serverSide: true,
				ajax: '{!! route('admin.user.delete.data') !!}',
				columns: [
					{ data: 'id', name: 'id' },
					{ data: 'name', name: 'name' },
			...
			$('#users-table tbody').on('click', '.btn_upback', function(event) {
				...
				Swal({
					...
					text: "Bạn chắc là muốn khôi phục người dùng này chứ!",
					type: 'warning',
				...
				Swal({
					...
					if (result.value) {
						Swal(
							...
							'Người Dùng đã bị xóa hoàn toàn khỏi hệ thống',
							'success'
							)
						$('button.swal2-confirm').on('click', function(event) {
							$.ajax({
