...
class AdminDeleteManagerController extends Controller
{
	/**
	...
	public function index() {
		return view('admin.admin-managers.delete.index');
	}
    ...
    public function getUserData() {
    	...
    	->addColumn('action', function ($admin) {
    		return '
            <a href="#user_detail" class="btn_detail" data-url="'.route('admin.admins.delete.detail',$admin->id).'" style="margin-right: 5px;" data-toggle="modal" title="Detail"><button class="btn btn-warning" id="user-detail" title="Detail" data-toggle="tooltip"><i class="glyphicon glyphicon-eye-open"></i></button></a>
            <a href="#user_edit" class="btn_edit" style="margin-right: 5px;" data-url="'.route('admin.admins.delete.edit',$admin->id).'" data-update="'.route('admin.admins.delete.update',$admin->id).'" data-updatepassword="'.route('admin.admins.delete.update.password',$admin->id).'" data-toggle="modal" title="Edit"><button class="btn btn-info" id="user-edit" title="Edit" data-toggle="tooltip"><i class="glyphicon glyphicon-edit"></i></button></a>
            <form method="POST" style="display: initial;" class="form_deactive_user">'.csrf_field().''.method_field('put').'<a href="#" class="btn_upback" style="margin-right: 5px;" data-url="'.route('admin.admins.delete.upback',$admin->id).'" title="Upback"><button class="user-upback btn btn-success" title="Upback" data-toggle="tooltip"><i class="fa fa-cloud-upload"></i></button></a></form>
            <form method="POST" style="display: initial;" class="form_delete_user">'.csrf_field().''.method_field('delete').'<a href="#" class="btn_delete" data-url="'.route('admin.admins.delete.realdelete',$admin->id).'" title="Delete"><button class="user-delete btn btn-primary" title="Delete" data-toggle="tooltip"><i class="glyphicon glyphicon-trash"></i></button></a></form>';
    	})
        ->editColumn('id', 'ID: {{$id}}')
    ...
    public function adminDeleteShow($id) {
    ...
    public function adminDeleteEdit($id) {
        $user = Admin::find($id);
    ...
    public function adminDeleteUploadImage(Request $request) {
    ...
    public function adminDeleteDeleteImage(Request $request) {
    ...
    public function adminDeleteStore(Request $request) {
    ...
    public function adminDeleteUpdate(Request $request, $id) {
        // dd($request->all());
        $user = Admin::find($id);
    ...
    public function adminDeleteUpdatePassword(Request $request, $id) {
    ...
    public function adminDeleteUpback($id) {
        $admin_active = Admin::find($id);
        if ($admin_active != null && $admin_active->delete_at == 0) {
            $admin_active->update([
                'delete_at' => 1,
            ]);

            return response()->json([
                'upback_success' => 'Khôi phục thành công người dùng '.$admin_active->name.' !!!'
            ]);
        } else {
            return response()->json([
                'upback_error' => 'Không thể khôi phục người dùng do không tìm thấy thông tin!!!'
            ]);
        }
    }

    public function adminDeleteRealDelete($id) {
        $admin_delete = Admin::find($id);
        $admin_pic_name = explode('/', $admin_delete->admin_pic);
        $user_name = $admin_delete->name;
        if ($admin_delete != null && $admin_delete->delete_at == 0) {
        	Storage::disk('user_file')->delete($admin_pic_name[2]);
            $admin_delete->delete();
