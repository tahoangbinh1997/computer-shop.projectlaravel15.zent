...
class UserDeactiveManagerController extends Controller
{
	/**
	...
	public function index() {
		// dd('Hello');
		return view('admin.user-managers.deactive.index');
	}
    ...
    public function getUserData() {
    	$user_deactive = User::where([
            ['active','=',0],
            ['delete_at','=',1]
        ])->orderBy('id','Desc');
    	return Datatables::of($user_deactive)
    	->addColumn('action', function ($user) {
    		return '
            <a href="#user_detail" class="btn_detail" data-url="'.route('admin.user.deactive.detail',$user->id).'" style="margin-right: 5px;" data-toggle="modal" title="Detail"><button class="btn btn-warning" id="user-detail" title="Detail" data-toggle="tooltip"><i class="glyphicon glyphicon-eye-open"></i></button></a>
            <a href="#user_edit" class="btn_edit" style="margin-right: 5px;" data-url="'.route('admin.user.deactive.edit',$user->id).'" data-update="'.route('admin.user.deactive.update',$user->id).'" data-updatepassword="'.route('admin.user.deactive.update.password',$user->id).'" data-toggle="modal" title="Edit"><button class="btn btn-info" id="user-edit" title="Edit" data-toggle="tooltip"><i class="glyphicon glyphicon-edit"></i></button></a>
            <form method="POST" style="display: initial;" class="form_active_user">'.csrf_field().''.method_field('put').'<a href="#" class="btn_active" style="margin-right: 5px;" data-url="'.route('admin.user.deactive.active',$user->id).'" title="Active"><button class="user-active btn btn-success" title="Active" data-toggle="tooltip"><i class="fa fa-unlock"></i></button></a></form>
            <form method="POST" style="display: initial;" class="form_delete_user">'.csrf_field().''.method_field('delete').'<a href="#" class="btn_delete" data-url="'.route('admin.user.deactive.delete',$user->id).'" title="Delete"><button class="user-delete btn btn-primary" title="Delete" data-toggle="tooltip"><i class="glyphicon glyphicon-trash"></i></button></a></form>';
    	})
        ->editColumn('id', 'ID: {{$id}}')
    ...
    public function userDeactiveShow($id) {
        $user_deactive_show = User::find($id);
        return response()->json([
            'data' => $user_deactive_show,
        ]);
    }

    public function userDeactiveEdit($id) {
        $user = User::find($id);
    ...
    public function userDeactiveUploadImage(Request $request) {
    ...
    public function userDeactiveDeleteImage(Request $request) {
    ...
    public function userDeactiveStore(Request $request) {
    ...
    public function userDeactiveUpdate(Request $request, $id) {
        $user = User::find($id);
    ...
    public function userDeactiveUpdatePassword(Request $request, $id) {
    ...
    public function userDeactiveActive($id) {
        ...
        if ($user_active != null && $user_active->active == 0) {
            ...
            return response()->json([
                'active_success' => 'Kích hoạt thành công người dùng '.$user_active->name.'. Thông tin người dùng sẽ được chuyển vào phần kích hoạt!!!'
            ]);
        } else {
            return response()->json([
                'active_error' => 'Không thể kích hoạt người dùng do không tìm thấy thông tin!!!'
            ]);
        }
    }

    public function userDeactiveDelete($id) {
