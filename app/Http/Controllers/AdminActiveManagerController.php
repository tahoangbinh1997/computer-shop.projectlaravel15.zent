...
class AdminActiveManagerController extends Controller
{
	/**
	...
	public function index() {
		return view('admin.admin-managers.active.index');
	}
    ...
    public function getUserData() {
    	...
    	->addColumn('action', function ($admin) {
    		return '
            <a href="#user_detail" class="btn_detail" data-url="'.route('admin.admins.active.detail',$admin->id).'" style="margin-right: 5px;" data-toggle="modal" title="Detail"><button class="btn btn-warning" id="user-detail" title="Detail" data-toggle="tooltip"><i class="glyphicon glyphicon-eye-open"></i></button></a>
            <a href="#user_edit" class="btn_edit" style="margin-right: 5px;" data-url="'.route('admin.admins.active.edit',$admin->id).'" data-update="'.route('admin.admins.active.update',$admin->id).'" data-updatepassword="'.route('admin.admins.active.update.password',$admin->id).'" data-toggle="modal" title="Edit"><button class="btn btn-info" id="user-edit" title="Edit" data-toggle="tooltip"><i class="glyphicon glyphicon-edit"></i></button></a>
            <form method="POST" style="display: initial;" class="form_deactive_user">'.csrf_field().''.method_field('put').'<a href="#" class="btn_deactive" style="margin-right: 5px;" data-url="'.route('admin.admins.active.deactive',$admin->id).'" title="Deactive"><button class="user-deactive btn btn-danger" title="Deactive" data-toggle="tooltip"><i class="glyphicon glyphicon-lock"></i></button></a></form>
            <form method="POST" style="display: initial;" class="form_delete_user">'.csrf_field().''.method_field('delete').'<a href="#" class="btn_delete" data-url="'.route('admin.admins.active.delete',$admin->id).'" title="Delete"><button class="user-delete btn btn-primary" title="Delete" data-toggle="tooltip"><i class="glyphicon glyphicon-trash"></i></button></a></form>';
    	})
        ->editColumn('id', 'ID: {{$id}}')
    ...
    public function adminActiveShow($id) {
    ...
    public function adminActiveEdit($id) {
        $user = Admin::find($id);
    ...
    public function adminActiveUploadImage(Request $request) {
    ...
    public function adminActiveDeleteImage(Request $request) {
    ...
    public function adminActiveStore(Request $request) {
    ...
    public function adminActiveUpdate(Request $request, $id) {
        // dd($request->all());
        $user = Admin::find($id);
    ...
    public function adminActiveUpdatePassword(Request $request, $id) {
    ...
    public function adminActiveDeactive($id) {
        $user_deactive = Admin::find($id);
        if ($user_deactive != null && $user_deactive->active == 1) {
            ...
            return response()->json([
                'deactive_success' => 'Hủy kích hoạt thành công nhân viên '.$user_deactive->name.'. Thông tin nhân viên sẽ được chuyển vào phần hủy kích hoạt!!!'
            ]);
        } else {
            return response()->json([
                'deactive_error' => 'Không thể hủy kích hoạt nhân viên do không tìm thấy thông tin!!!'
            ]);
        }
    }

    public function adminActiveDelete($id) {
